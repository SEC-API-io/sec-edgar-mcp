...
The capture asked for `owners.name:Point72 AND owners.amountAsPercent:[10 TO *]`
with `size: 1`. It returned `total.value: 8` and one SC 13D from 2022 in 1,319
...
| --------- | ------- | -------- | ---------------------- | ----------------------------------------------------------- |
| `query`   | string  | yes      | max 1,000 characters   | Lucene syntax. See [query language](../query-language.md). This handler does not require a `:`, unlike its siblings. |
...
ranges in the query. The search runs across the 13D and 13G indices together, so
one query returns both form families. Query fields:

- `owners.name`. Confirmed. The capture used `owners.name:Point72`.
- `owners.amountAsPercent`. Confirmed, including range syntax. The capture used
  `owners.amountAsPercent:[10 TO *]`.
- `accessionNo`. Used in the Node SDK example as `accessionNo:*`, **unverified**
  through MCP.
- `formType`, `nameOfIssuer`, `cusip`, `filers.cik`, `eventDate`, `filedAt`,
  `titleOfSecurities`. All present in the response body, all **unverified** as
  query fields.
...
| `formType`                        | string  | `SC 13D` in the capture. The SDK response also shows `SC 13D/A`.  |
...
| `owners[].name`                   | string  | **Type varies.** A string in the capture, an array of strings in the canonical SDK response. Handle both. |
| `owners[].sourceOfFunds`          | array   | **Type varies.** `["OO"]` in the capture, `"OO"` in the SDK response. |
...
| `owners[].aggregateAmountOwned`, `.amountAsPercent` | number | Total beneficially owned shares, and percent of the class. `5351000` and `20.3` in the capture. |
| `owners[].typeOfReportingPerson`  | array   | SEC codes. Observed: `IN` individual, `CO` corporation, `OO` other. Not the full code list. |
| `owners[].place`                  | string  | Place of organisation or citizenship. `Delaware` and `United States` in the capture. The SDK response uses codes such as `X0` and `D8`. |
| `owners[].amountExcludesCertainShares` | boolean | **Field name varies.** The capture uses this name. The canonical SDK response uses `isAggregateExcludeShares`. Check for both. |
| `item1` to `item7`                | object  | Narrative items: security and issuer, identity, source of funds, purpose, interest in securities, contracts, exhibits. **Absent in the capture.** Present in the canonical SDK response for an `SC 13D/A`. Whether they appear on every record is not verified. |

Add the `owners[]` numbers with care. In the capture both reporting persons
report the same 5,351,000 shares and the same 20.3%. That is one economic
  ...
  "filings": [
    {
      ...
      "filers": [
        { "cik": "1813658", "name": "Tempo Automation Holdings, Inc. (Subject)" },
        { "cik": "1954961", "name": "Point72 Private Investments, LLC (Filed by)" }
      ],
...
Trimmed. The capture also holds `titleOfSecurities`, `cusip`, `eventDate`,
