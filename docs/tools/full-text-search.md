...
The capture returned 100 rows that held only 84 distinct accession numbers.
Results come back by relevance, never by date, so `filedAt` values arrive out of
order. The search covers only the last 12 months unless you give a date range.
...
| ----------- | ---------------- | -------- | ---------------- | ------------------------------------------------------------------------------ |
| `query`     | string           | yes      | Search operators | Quotes make an exact phrase. `OR` and wildcards are documented but unverified. |
...
| `startDate` | string           | no       | `YYYY-MM-DD`     | Use it **with** `endDate`. See the warning below.                              |
| `endDate`   | string           | no       | `YYYY-MM-DD`     | Use it **with** `startDate`.                                                   |
...
`ticker:AAPL` here. Use `ciks` and `formTypes` to filter. The only verified query
form is a quoted phrase, `"quantum computing"`, from the capture. The `OR` and
wildcard operators come from the tool description and are unverified.

**Give both dates or neither.** The server applies your range only when
`startDate` and `endDate` are both set. If either one is missing it overwrites
both with a window of the last 12 months, silently. This comes from the server
handler. The capture shows the effect: with no dates at all, the 100 rows span
`2025-08-13` to `2026-08-12`, and the probe ran on 2026-08-13.
...
| `filings[].companyNameLong` | string         | Filer name. The capture shows `Quantum Computing Inc. (Filer)`. The SDK example shows the older `Lipocine Inc. (LPCN) (CIK 0001535955)` form. |
| `filings[].ticker`          | string or null | Ticker of the filer. It is `null` when no ticker maps. 7 of the 100 captured rows were `null`.                                                |
...
first. Sort the rows yourself after the call. The captured 100-row page was
31,531 bytes, so budget about 32 KB per page.
...
matches at 100 rows per page means 5 calls. Ask for `page: 2` next.

## Limits and errors

- Coverage starts in 2001. Older filings are invisible here.
- The default date window is the last 12 months. Set both `startDate` and
  `endDate` to go further back.
- Count filings by unique `accessionNo`, not by row. `total.value` counts
  documents, so it overstates the number of filings.
- The query string has a 2,000 character ceiling in the server handler, with the
  message `Query too long. Maximum length: 2000 characters`. Not verified
  through MCP. Note that this is a tighter limit than the 3,500 characters of
